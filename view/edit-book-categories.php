        ...
        <?php include 'alert.php'; ?>  
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="white-box">
                            <h3 class="box-title m-b-0">Mengedit Kategori Buku</h3>
                            <p class="text-muted m-b-30 font-13"> Isi form dengan baik dan benar! </p>
                            <p class="text-right">
                                <a href="<?=$host?>/book_category.php?action=deleteBookCategory&id=<?=$_GET['id']?>&mode=PHP" class="btn btn-danger">Hapus Data</a>
                            </p>
                            <?php // Get Target Data to be Edited
                                include './request-links.php';
                                $url = $host.'/book_category.php?action=selectBookCategoryByID&id='.$_GET['id'].'&mode=JSON';
                                $result = fetchJsonFromApi($url);
                                if($result['response']!= 200){ // If no data
                                    header('Location: ./show-book-categories.php');
                                }
                                $data = $result['data'][0];
                            ?>
                            <form class="form-horizontal" action="<?=$host?>/book_category.php" method="POST">                                                                                                                        
                                ...
                                <div class="container-flex">
                                    <p class="text-center">
                                        ...
                                        <input type="hidden" name="action" value="updateBookCategory">
