        ...
        <?php include 'alert.php'; ?>  
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="white-box">
                            <h3 class="box-title m-b-0">Menambahkan Pengembalian Baru</h3>
                            <p class="text-muted m-b-30 font-13"> Isi form dengan baik dan benar! </p>
                            <form class="form-horizontal" action="<?=$host?>/return.php" method="POST">
                                <?php
                                // Lend data                               
                                $url = $host.'/lend.php?action=selectLendByID&mode=JSON&id='.$_GET['lend_id'];
                                $lend_data = fetchJsonFromApi($url)['data'][0];
                                ?>
                                <div class="form-group">    
                                    <label>Member Peminjam</label>                                  
                                    <input type="text" class="form-control" value="<?= $lend_data['member'] ?>" disabled>
                                </div>       
                                <div class="form-group">    
                                    <label>Buku Dipinjam</label>                                  
                                    <input type="text" class="form-control" value="<?= $lend_data['book'] ?>" disabled>
                                </div>  
                                <div class="form-group">    
                                    <label>Yang Dipinjam Tanggal</label>                                  
                                    <input type="text" class="form-control" value="<?= $lend_data['date'] ?>" disabled>
                                </div>    
                                <div class="form-group">    
                                    <label>Sampai Tanggal</label>                                  
                                    <input type="text" class="form-control" value="<?= $lend_data['will_return'] ?>" disabled>
                                </div>                               
                                <div class="form-group">    
                                ...
                                <div class="container-flex">
                                    <p class="text-center">
                                        ...
                                        <input type="hidden" name="member" value="<?= $lend_data['member_id'] ?>">
                                        <input type="hidden" name="book" value="<?= $lend_data['book_id'] ?>">
                                        <input type="hidden" name="lend_id" value="<?= $lend_data['id'] ?>">
