        ...
        <?php include 'alert.php'; ?>        
            <div class="container-fluid">                
                <div class="row">
                    <div class="col-sm-12">
                        <div class="white-box">
                            <h3 class="box-title m-b-0">Data Peminjaman Buku</h3>                            
                            <div class="table-responsive">
                                <table id="myTable" class="table table-striped">
                                    ...
                                    <tbody>
                                    <?php
                                    $url = $host.'/lend.php?action=selectLendAll&mode=JSON';
                                    $fetchResult = fetchJsonFromApi($url);
                                    if($fetchResult['response'] == 200){
                                        for($i=0; $i<count($fetchResult['data']);$i++){ 
                                            $row = $fetchResult['data'][$i];
                                            $stat = array('Dipinjam', 'Dikembalikan');
                                            $deletable = array('', 'hidden');
                                        ?>                                                                                
                                        <tr>                                                                                       
                                            <td><?= $row['date'] ?></td>                                                  
                                            <td><?= $row['member'] ?></td>     
                                            <td><?= $row['book'] ?></td>     
                                            <td><?= $row['will_return'] ?></td>     
                                            <td><?= $stat[$row['returned']] ?></td>   
                                            <td>
                                                <a href="../service/lend.php?id=<?= $row['id'] ?>&book=<?= $row['book_id'] ?>&action=deleteLend&mode=PHP" <?= $deletable[$row['returned']] ?>><i class="icon-trash"></i> Kesalahan </a><br>
                                            </td>  
                                        </tr>  
                                    <?php }
                                    }
