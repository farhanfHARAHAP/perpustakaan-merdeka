...
session_start();
if(!isset($_GET['member_id']) || !isset($_GET['book_id'])){
        ...
        <?php include 'alert.php'; ?>  
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="white-box">
                            <h3 class="box-title m-b-0">Menambahkan Peminjaman Baru</h3>
                            <p class="text-muted m-b-30 font-13"> Isi form dengan baik dan benar! </p>
                            <form class="form-horizontal" action="<?=$host?>/lend.php" method="POST">
                                <?php
                                // Peminjam                                
                                $url = $host.'/member.php?action=selectMemberByID&mode=JSON&id='.$_GET['member_id'];
                                $peminjam = fetchJsonFromApi($url)['data'][0]['name'];
                                ?>
                                <div class="form-group">    
                                    <label>Member Peminjam</label>                                  
                                    <input type="text" class="form-control" value="<?= $peminjam ?>" disabled>
                                </div>  
                                <?php
                                // Peminjam                                
                                $url = $host.'/book.php?action=selectBookByID&mode=JSON&id='.$_GET['book_id'];
                                $peminjam = fetchJsonFromApi($url)['data'][0]['name'];
                                ?>
                                <div class="form-group">    
                                ...
                                <div class="form-group">    
                                    ...
                                    <input type="text" class="form-control" name="return" placeholder="YYYY-MM-DD" required>
                                </div>                                                                                         
                                <div class="container-flex">
                                    <p class="text-center">
                                        <input type="hidden" name="id" value="null">
                                        <input type="hidden" name="returned" value="null">
                                        <input type="hidden" name="member" value="<?= $_GET['member_id'] ?>">
                                        <input type="hidden" name="book" value="<?= $_GET['book_id'] ?>">
                                        <input type="hidden" name="mode" value="PHP">
                                        <input type="hidden" name="action" value="insertLend">
